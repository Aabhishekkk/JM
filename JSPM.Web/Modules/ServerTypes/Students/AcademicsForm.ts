...
export class AcademicsForm extends PrefixedContext {
    static formKey = 'Students.Academics';
    private static init: boolean;

    constructor(prefix: string) {
        super(prefix);

        if (!AcademicsForm.init)  {
            AcademicsForm.init = true;

            var w0 = StringEditor;
            var w1 = IntegerEditor;

            initFormType(AcademicsForm, [
                'CourseLevel', w0,
                'CourseName', w0,
                'YearOfPassing', w0,
                'Percentage', w0,
