   ...
   [FromServices] IProjectSaveHandler handler)
    {
        ...
        for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
        {
            try
            {
                ...
                Row.SkillCategory = Convert.ToString(worksheet.Cells[row, 1].Value ?? "").Trim();
                if (string.IsNullOrEmpty(Row.SkillCategory))
                {
                    response.ErrorList.Add("Error On Row " + row + ": SkillCategory Not found");
                    continue;
                }
                ...
                if (!string.IsNullOrEmpty(SkillId))
                {
                    ...
                    {
                        response.ErrorList.Add("Error On Row " + row + ": Invalid SkillId!");
                        continue;
                    }
                }
                ...
                Row.SkillName = Convert.ToString(worksheet.Cells[row, 7].Value ?? "").Trim();
                if (string.IsNullOrEmpty(Row.SkillName))
                {
                    response.ErrorList.Add("Error On Row " + row + ": SkillName Not found");
                    continue;
                }
